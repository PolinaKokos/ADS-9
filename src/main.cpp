...
int main() {
    ...
    for (int n : sizes) {
        ...
        double timeAll = measureTime([](const PMTree& t, int) { 
            getAllPerms(t); 
            return vector<char>(); 
        }, n, -1);
    ...
    if (gnuplotScript.is_open()) {
        gnuplotScript << "set terminal pngcairo size 800,600 enhanced font 'Verdana,10'\n";
        ...
        gnuplotScript << "plot 'data.txt' using 1:2 with linespoints title 'getAllPerms' lw 2, \\\n";
        gnuplotScript << "     'data.txt' using 1:3 with linespoints title 'getPerm1 (slow)' lw 2, \\\n";
        gnuplotScript << "     'data.txt' using 1:4 with linespoints title 'getPerm2 (fast)' lw 2\n";
        gnuplotScript.close();
    ...
    cout << "3. getPerm2(): factorial complexity O(n!) with smaller constant" << endl;
    cout << "\nRecommendation: Use getPerm2() for individual permutations." << endl;
