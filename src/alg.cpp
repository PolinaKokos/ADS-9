...
void PMTree::buildTree(std::shared_ptr<Node> currentNode, std::vector<char> remaining) {
    if (remaining.empty()) {
        return;
    }
...
void PMTree::collectPermutations(std::shared_ptr<Node> node, std::vector<char>& current, 
...
int PMTree::getTotalPermutations() const {
    int total = 1;
    for (int i = 2; i <= originalSet.size(); i++) {
        total *= i;
    }
    return total;
}
...
std::vector<char> PMTree::getPermByNavigation(int num) const {
    ...
    for (size_t i = 0; i < originalSet.size(); i++) {
        int blockSize = 1;
        for (int j = 1; j <= available.size() - 1; j++) {
            blockSize *= j;
        }
